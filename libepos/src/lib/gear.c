...
#ifdef __ELMO__	// ELMO
  return 2.0*M_PI*velocity/(8000.0*gear->transmission);
#else	// EPOS
...
#ifdef __ELMO__ // ELMO
  return clip(angular_vel*8000.0*gear->transmission/(2.0*M_PI),
    INT_MIN, INT_MAX);
#else	// EPOS
...
#ifdef __ELMO__ // ELMO
  return 2.0*M_PI*acceleration/(8000.0*gear->transmission);
#else	// EPOS
...
#ifdef __ELMO__ // ELMO
  return clip(angular_acc*8000.0*abs(gear->transmission)/(2.0*M_PI), 
    INT_MIN, INT_MAX);
#else	// EPOS
