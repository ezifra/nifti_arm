  ...
  if (!can_receive_message(dev->can_dev, message)) {
    if ((message->id >= EPOS_DEVICE_EMERGENCY_ID) &&
      ...
      if (code != EPOS_DEVICE_ERROR_NONE) {
        fprintf(stderr, "Node %d device error: [0x%hX] %s (%x:%x:%x:%x:%x:%x:%x:%x).\n",
...
int epos_device_write(epos_device_p dev, short index, unsigned char subindex,
  ...
  switch (num) {
    case 1 : message.content[0] = EPOS_DEVICE_WRITE_SEND_1_BYTE;
  			 printf("device.c::epos_device_write: 0x%04x:%x [%02x].\n", index, subindex, data[0]);
             break;
    case 2 : message.content[0] = EPOS_DEVICE_WRITE_SEND_2_BYTE;
  			 printf("device.c::epos_device_write: 0x%04x:%x [%02x:%02x].\n", index,
			 subindex, data[1], data[0]);
             break;
    case 4 : message.content[0] = EPOS_DEVICE_WRITE_SEND_4_BYTE;
			 printf("device.c::epos_device_write: 0x%04x:%x [%02x:%02x:%02x:%02x].\n",
			 index, subindex, data[3], data[2], data[1], data[0]);
             break;
  ...
  if (!epos_device_send_message(dev, &message) &&
    !epos_device_receive_message(dev, &message)) {
	printf("\t-> 0x%02x%02x:%x [%02x:%02x:%02x:%02x](%02x).\n", message.content[2],
		message.content[1], message.content[3], message.content[7],
		message.content[6], message.content[5], message.content[4],
		message.content[0]);
    ++dev->num_written;
    return EPOS_DEVICE_ERROR_NONE;
  }
