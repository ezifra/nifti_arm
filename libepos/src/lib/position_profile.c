...
*/
  ...
  printf("epos_control_set_type(%d)...\n", epos_control_profile_pos);fflush(stdout);
  if (result = epos_control_set_type(&node->control,
 			 epos_control_profile_pos)) {
    printf("\terror (%d).\n", result);
	return result;
  ...
  printf("epos_position_profile_set_velocity (%d)...\n", vel);fflush(stdout);
  if (result = epos_position_profile_set_velocity(&node->dev, vel)) {
    printf("\terror (%d).\n", result);
	return result;
  } else
  	printf("\tOK (%d).\n", result);

  printf("epos_profile_set_acceleration (%d)...\n", acc);fflush(stdout);
  if (result = epos_profile_set_acceleration(&node->dev, acc)) {
    printf("\terror (%d).\n", result);
	return result;
  } else
  	printf("\tOK (%d).\n", result);

  printf("epos_profile_set_deceleration (%d)...\n", dec);fflush(stdout);
  if (result = epos_profile_set_deceleration(&node->dev, dec)) {
    printf("\terror (%d).\n", result);
	return result;
  } else
  	printf("\tOK (%d).\n", result);

  printf("epos_profile_set_type (%d)...\n", profile->type);fflush(stdout);
  if (result = epos_profile_set_type(&node->dev,
 			 profile->type)) {
    printf("\terror (%d).\n", result);
	return result;
  } else
  ...
  printf("epos_position_profile_set_target (%d)...\n", pos);fflush(stdout);
  if (result = epos_position_profile_set_target(&node->dev, pos)) {
    printf("\terror (%d).\n", result);
	return result;
  } else
  	printf("\tOK (%d).\n", result);

  printf("epos_device_set_control (%x)...\n", control);fflush(stdout);
  if (result = epos_device_set_control(&node->dev, control)) {
    printf("\terror (%d).\n", result);
	return result;
  } else
  	printf("\tOK (%d).\n", result);

  printf("epos_control_start...\n");fflush(stdout);
  if (result = epos_control_start(&node->control)) {
    printf("\terror (%d).\n", result);
	return result;
  } else
  	printf("\tOK (%d).\n", result);
